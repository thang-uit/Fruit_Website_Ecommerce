...
<?php
    if(isset($_SESSION['loginadmin']))
    {
        ...
        $SQLOrder = "SELECT * FROM `orders`, `customer` WHERE `orders`.`Cus_ID` = `customer`.`Cus_ID` AND `orders`.`Ord_Status` = '2' LIMIT " . $item_per_page . " OFFSET " . $offset;
        $Order = $con->query($SQLOrder);
        
        $total_records = mysqli_query($con, "SELECT * FROM `orders`, `customer` WHERE `orders`.`Cus_ID` = `customer`.`Cus_ID` AND `orders`.`Ord_Status` = '2'");
...
?>
            ...
            <table border="1" cellspacing="0" align="center" id="loadorder">
                ...
                ?>
                        <tr>
                            ...
                            <td><?= $row['Ord_Date'] ?></td>
                            <td><a href="Order_Detail.php?ODID=<?= $row['Ord_ID'] ?>&CUSID=<?= $row['Cus_ID'] ?>" target="_blank" onclick="window.open(this.href, 'order', 'left=500, top=50, width=700, height=600, toolbar=1, resizable=0'); return false;" style="color: #06ba2a;">Print Order</a></td>
                            <td>
                    ...
                    <td colspan="10">
                        ...
                        <?php
                            if($current_page > 5)
                            {
                                $first_page = 1;                 
                        ?>
                                <a href="?action=order3&page=<?= $first_page ?>"><span><strong>First</strong></span></a>
                        <?php
                            }
                        ?>
                        ...
                        <?php
                            for($num = 1; $num <= $total_page; $num++)
                            {
                                if($num != $current_page)
                                {
                                    ...
                                    {
                        ?>
                                        <a href="?action=order3&page=<?= $num ?>"><span><?= $num ?></span></a>
                        <?php
                                    }  
                                }
                                ...
                                {
                        ?>
                                    <a href="?action=order3&page=<?= $num ?>"><span class="select"><?= $num ?></span></a>
                        <?php
                                } 
                            }
                        ?>
                        ...
                        <?php
                            if($current_page < $total_page - 5)
                            {
                                $end_page = $total_page;
                        ?>
                                <a href="?action=order3&page=<?= $end_page ?>"><span><strong>Last</strong></span></a>
                        <?php
                            }
                        ?>
                        </div>
        ...
        <script type="text/javascript">
            function Confirm_Order(id)
            {
                $.ajax
                ({
                    url: "Order_Process.php?action=order3", // gửi ajax đến file
                    ...
                    {                       
                        alert(result);
                        $("#loadorder").load("Admin_Page.php?action=order3 #loadorder");
                    }
                });
            }

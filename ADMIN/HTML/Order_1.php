...
<?php
    if(isset($_SESSION['loginadmin']))
    {
        ...
        $SQLOrder = "SELECT * FROM `orders`, `customer` WHERE `orders`.`Cus_ID` = `customer`.`Cus_ID` AND `orders`.`Ord_Status` = '0' LIMIT " . $item_per_page . " OFFSET " . $offset;
        $Order = $con->query($SQLOrder);
        
        $total_records = mysqli_query($con, "SELECT * FROM `orders`, `customer` WHERE `orders`.`Cus_ID` = `customer`.`Cus_ID` AND `orders`.`Ord_Status` = '0'");
...
?>
        <div class="order1">
            <h1><i class="fa fa-cart-plus"></i> New Order</h1>
            <br>


            <div class="table-container">
                <input type="text" class="search-od" name="search" id="search-od" placeholder="Customer name">
                <br> <br>
                            ...
                            <td colspan="9">
                                ...
                                <?php
                                    if($current_page > 5)
                                    {
                                        $first_page = 1;                 
                                ?>
                                        <a href="?action=order1&page=<?= $first_page ?>"><span><strong>First</strong></span></a>
                                <?php
                                    }
                                ?>
                                ...
                                <?php
                                    for($num = 1; $num <= $total_page; $num++)
                                    {
                                        if($num != $current_page)
                                        {
                                            ...
                                            {
                                ?>
                                                <a href="?action=order1&page=<?= $num ?>"><span><?= $num ?></span></a>
                                <?php
                                            }  
                                        }
                                        ...
                                        {
                                ?>
                                            <a href="?action=order1&page=<?= $num ?>"><span class="select"><?= $num ?></span></a>
                                <?php
                                        } 
                                    }
                                ?>
                                ...
                                <?php
                                    if($current_page < $total_page - 5)
                                    {
                                        $end_page = $total_page;
                                ?>
                                        <a href="?action=order1&page=<?= $end_page ?>"><span><strong>Last</strong></span></a>
                                <?php
                                    }
                                ?>
                                </div>
        ...
        <script type="text/javascript">
            function Confirm_Order(id)
            {
                $.ajax
                ({
                    url: "Order_Process.php?action=order1", // gửi ajax đến file
                    ...
                    {                       
                        alert(result);
                        $("#loadorder").load("Admin_Page.php?action=order1 #loadorder");
                    }
                });
            }
            ...
            $(function()
            {
                $("#search-od").keyup(function()
                {
                    var keyword = $("#search-od").val();
                    $.ajax
                    ({
                        url: "Order_Process.php?action=search", // gửi ajax đến file
                        type: "post", // chọn phương thức gửi là get
                        dateType: "text", // dữ liệu trả về dạng text
                        data: 
                        { 

<?php
    if(isset($_SESSION['login']))
    {
        ...
        $SQLOrder = "SELECT * FROM `orders`, `customer` WHERE `orders`.`Cus_ID` = `customer`.`Cus_ID` AND `orders`.`Ord_Status` = '2' AND `orders`.`Cus_ID` = '$ID_Cus'";
        $Order = $con->query($SQLOrder);

        $total = mysqli_num_rows($Order);   
        $_SESSION['Order2'] = $total;  
?>
                    ...
                    <table border="1" cellspacing="0" align="center" id="loadorder">   
                        ...
                        ?>
                                <tr class="table-content">
                                    ...
                                    <td>
                                        <table border="1" cellspacing="0">
                                            ...
                                            <?php
                                                ...
                                                $ID_Order = $row['Ord_ID'];
                                                $SQLOrderDT = "SELECT * FROM `orders`, `order_details`, `products` WHERE `orders`.`Ord_ID` = `order_details`.`Ord_ID` AND `order_details`.`Pro_ID` = `products`.`Pro_ID` AND `orders`.`Ord_Status` = '2' AND `orders`.`Ord_ID` = '$ID_Order' AND `orders`.`Cus_ID` = '$ID_Cus'";
                                    ...
                                    <td>
                                        <h4 style="color: #32a852; font-weight: bolder;">Đang Giao Hàng</h4>
                                    </td>                            
                                </tr>
                        <?php
                            }
